...
public interface IMonAnSvc
{
    ...
    //Task<List<MonAn>> SearchMonans(string search);
}
...

{
    ...
    //public async Task<List<MonAn>> SearchMonans(string search)
    //{
    //    return await _context.MonAns
    //        .Where(p => p.Name.Contains(search) || p.MoTa.Contains(search))
    //        .ToListAsync();
